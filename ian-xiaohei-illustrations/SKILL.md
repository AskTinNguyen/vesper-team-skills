...
description: "Use for Xiaohei-style Chinese article illustrations: plan shot lists, generate 16:9 hand-drawn explanatory images, or clean/edit generated Xiaohei images while preserving attribution and style constraints."
...
The leading word is **weird anchor**: each image needs one strange-but-valid metaphor that explains the article idea, with Xiaohei participating in the action rather than decorating the scene.

## Choose the branch

- **Strategy branch** — digest the source and produce a shot list only; stop unless the user asks to generate.
- **Generation branch** — digest source, build prompts from the template, generate images, QA them, then save/deliver.
- **Cleanup branch** — inspect an existing image, remove title/noise/crowding/style drift, QA, then save as a new artifact.

Completion criteria:

- Strategy is done when each proposed image has placement, cognitive anchor, scene structure, Xiaohei action, visual elements, and label plan.
- Generation is done when every delivered image passes QA or failures are disclosed.
- Cleanup is done when the requested defect is fixed without overwriting the original and the output path is reported.

## Safety and rights note

This skill is adapted from the MIT-licensed public repository `helloianneo/ian-xiaohei-illustrations`. Keep attribution when redistributing. Treat outputs as inspired by/adapted from the public Xiaohei illustration system; do not present generated images as official Ian artwork unless Ian made them. Use the examples only for style calibration, not for direct copying.
