...
description: "Use for agent-native HTML-to-video work: new repo setup, existing repo render/export, failed-render troubleshooting, local studio verification, Playwright/Chromium capture, ffmpeg, and MP4 smoke tests."
version: 1.0.0
author: Hermes Agent
license: MIT
...
Use this skill when the user asks to install, set up, troubleshoot, render, export, or smoke-test an agent-native HTML-to-video project/tool (for example an HTML/CSS/JS + template gallery + local studio + MP4 export workflow). The goal is not just dependency installation: always verify the actual render path end-to-end when possible.

## Choose the branch

- **New repo/tool setup** — clone/update, install, build, install render prerequisites, then run a real MP4 smoke test.
- **Existing repo render/export** — use the repo's declared CLI or studio path and finish only after the output file exists and is playable/non-empty.
- **Troubleshooting failed render** — inspect template variables, browser install, ffmpeg availability, CLI logs, and retry the shortest render.
- **Studio verification** — start the local studio and finish only after the HTTP endpoint responds and at least one template can be previewed or rendered.

Do not report success from dependency installation alone; the render path is the product.
