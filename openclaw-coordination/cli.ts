...
import type { CoordinationState, Dirs, AgentSession, AgentActivity } from "./types.js";
import {
  register,
  unregister,
  getActiveAgents,
...
function createState(agentName: string): CoordinationState {
  const now = new Date().toISOString();
  const session: AgentSession = { toolCalls: 0, tokens: 0, filesModified: [] };
  const activity: AgentActivity = { lastActivityAt: now };
...
async function cmdReserve(args: string[]): Promise<void> {
  ...
  try {
    ...
    const conflicts: Array<{ agent: string; pattern: string; file: string }> = [];
