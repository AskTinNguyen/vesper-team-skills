...
import { claimTask, getClaims, type ClaimResult } from "../claims.js";
...
function cleanupDirs(dirs: Dirs): void {
...
describe("claims: race condition - concurrent claiming", () => {
  ...
  it("should allow only 1 of 5 concurrent claims to succeed", async () => {
    ...
    const promises: Promise<ClaimResult & { agent: string }>[] = [];
    for (let i = 1; i <= 5; i++) {
      const agent = `agent${i}`;
      const sessionKey = `session-${i}`;
      promises.push(
        claimTask(dirs, specPath, taskId, agent, sessionKey, `Attempt ${i}`)
    ...
    for (const failure of failures) {
      ...
      if (!failure.success) {
        assert.ok(
          failure.error === "already_claimed" || failure.error === "already_have_claim",
          `Expected failure error to be "already_claimed" or "already_have_claim", got "${failure.error}"`
        );
      }
    }
  ...
  it("should handle 10 concurrent claims with no data corruption", async () => {
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      const sessionKey = `session-${i}`;
      promises.push(
        claimTask(dirs, specPath, taskId, agent, sessionKey)
          .then(result => ({ ...result, agent }))
      );
    }
  ...
  it("should handle concurrent claims on different tasks", async () => {
    ...
    const promises: Promise<ClaimResult & { agent: string; taskId: string }>[] = [];
    for (let i = 1; i <= 5; i++) {
      const agent = `agent${i}`;
      const taskId = `TASK-${i}`;
      const sessionKey = `session-${i}`;
  ...
  it("should be stable across multiple race rounds", async () => {
    ...
    for (let round = 1; round <= 3; round++) {
      ...
      for (let i = 1; i <= 5; i++) {
        promises.push(
          claimTask(dirs, specPath, taskId, `agent${i}`, `session-${i}`)
        );
      }
      ...
      assert.strictEqual(
        successes.length,
        1,
        `Round ${round}: exactly one success expected`
      );
    }
