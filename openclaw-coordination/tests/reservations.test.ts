...
import type { CoordinationState, Dirs, AgentRegistration } from "../types.js";
...
function createTestState(name: string): CoordinationState {
  return {
    agentName: name,
...
function createMockAgent(
  dirs: Dirs,
  name: string,
  reservations: Array<{ pattern: string; reason?: string }>
): void {
...
describe("reservations: addReservations / removeReservations", () => {
  ...
  it("should add multiple reservations", () => {
    ...
    assert.deepStrictEqual(
      state.reservations.map(r => r.pattern),
      ["src/api/", "package.json", "src/utils/"]
    );
  });
