...
description: "Manage Hermes Agent skill libraries as a class-level system: import upstream skills, normalize metadata/support files, curate and consolidate local skills, and verify safe discoverability."
version: 1.0.0
author: Hermes Agent
license: MIT
...
## Windows/media path pitfall during skill work

On native Windows Hermes gateway sessions, generated artifacts sent through messaging adapters may cross Git-Bash/MSYS and native Python path boundaries. Prefer gateway-parseable Windows paths such as `MEDIA:C:/Users/Admin/path/file.png` or the current adapter-supported canonical form; avoid assuming `/c/Users/...` will be accepted by Python adapters.
