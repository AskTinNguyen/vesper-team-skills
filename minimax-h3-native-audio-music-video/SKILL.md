...
Read [references/upstream-and-runtime.md](references/upstream-and-runtime.md) when installing, updating, debugging nodes, or checking provenance.
...
5. Keep `MiniMaxH3NativeAudioLock` between `MiniMaxH3ReferenceToVideo` and the sampler.
6. Feed the node's `exact_audio` output to `CreateVideo`. Do not use the generated audio decode for the final mux.
7. Keep the Ref2VA INT8 model and the installed Heretic MiniMax encoder selected.
8. Render one take, inspect motion and audio sync, and accept or reject before continuing.
...
Use one character reference for each new scene. Start with:

```text
Use <Picture 1> as the exact character identity and scene anchor: [identity, outfit, setting].
[One principal action] with [one camera movement].
[Vocal instruction or non-vocal mouth instruction].
Maintain [locked palette, lighting, time of day, and background].
No [known identity drift, extra people, text, logos, lighting changes, or environmental morphing].
End on [simple stable final-frame composition] and hold nearly still.
```

For vocal timestamps, write: `sing the exact supplied vocal audio with natural, precise lip synchronization.`
